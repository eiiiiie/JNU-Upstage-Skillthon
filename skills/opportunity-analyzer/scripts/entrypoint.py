...
def main() -> None:
    ...
    사용법:
        python entrypoint.py <파일1> [파일2 ...] [옵션]

    옵션:
        --api-key, -k   Upstage API 키 (미설정 시 UPSTAGE_API_KEY 환경변수 사용)
        --학년          학년 (예: 2학년)
        --학적          학적 상태 (재학 / 휴학 / 졸업예정)
        --목표          활동 목표, 쉼표 구분 (예: "포트폴리오 강화,상금")
        --역량          보유 역량 (예: "파이썬 가능,팀 프로젝트 경험")
        --일정          활동 가능 조건 (예: "여름방학 가능")

    예시:
        python entrypoint.py 공고.pdf --학년 2학년 --학적 재학 --목표 "포트폴리오 강화,상금"
    """
    import argparse

    parser = argparse.ArgumentParser(
        description="opportunity-analyzer: 공고 문서를 분석해 지원 적합도를 판단합니다.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "files",
        nargs="+",
        help="분석할 공고 파일 경로 (PDF 또는 이미지, 복수 가능)",
    )
    parser.add_argument(
        "--api-key", "-k",
        dest="api_key",
        default="",
        help="Upstage API 키 (미설정 시 UPSTAGE_API_KEY 환경변수 사용)",
    )
    parser.add_argument("--학년",  dest="학년",  default="", help="학년 (예: 2학년)")
    parser.add_argument("--학적",  dest="학적",  default="", help="학적 상태 (재학/휴학/졸업예정)")
    parser.add_argument("--목표",  dest="목표",  default="", help="활동 목표, 쉼표 구분 (예: 포트폴리오 강화,상금)")
    parser.add_argument("--역량",  dest="역량",  default="", help="보유 역량 (예: 파이썬 가능,팀 프로젝트 경험)")
    parser.add_argument("--일정",  dest="일정",  default="", help="활동 가능 조건 (예: 여름방학 가능)")

    parsed = parser.parse_args()

    # API 키: 인자 우선, 없으면 환경변수
    api_key = parsed.api_key or os.environ.get("UPSTAGE_API_KEY", "")
    if not api_key:
        print(
    ...
    file_paths: list[str] = parsed.files
    ...
    # CLI 인자로 받은 사용자 정보를 딕셔너리로 구성
    # 값이 있는 항목만 포함 → collect_user_info()가 이미 채워진 항목은 묻지 않음
    user_info: dict = {}
    if parsed.학년:
        user_info["학년"] = parsed.학년
    if parsed.학적:
        user_info["학적"] = parsed.학적
    if parsed.목표:
        user_info["목표"] = [v.strip() for v in parsed.목표.split(",") if v.strip()]
    if parsed.역량:
        user_info["역량"] = [v.strip() for v in parsed.역량.split(",") if v.strip()]
    if parsed.일정:
        user_info["일정"] = parsed.일정

    # 모든 필수 항목(학년, 학적, 목표)이 CLI로 전달됐으면 collect_user_info 건너뜀
    _required_keys = {"학년", "학적", "목표"}
    if _required_keys.issubset(user_info.keys()):
        final_user_info = user_info
    else:
        # 일부 누락 시 collect_user_info로 보완 (이미 있는 항목은 묻지 않음)
        final_user_info = collect_user_info(user_info)

    output = run_pipeline(file_paths, api_key, user_info=final_user_info)
    print(output)
